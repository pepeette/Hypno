...
class UniversalProfilerEnhanced:
    """
    Enhanced Universal Profiler that properly handles healthy cases
    Integrates HealthyCaseProfiler for adaptive-range individuals
    """
    
    def __init__(self):
        self.universal_profiler = UniversalProfiler()
        self.healthy_profiler = HealthyCaseProfiler()
        self.clinical_threshold = 4.0
    
    def create_universal_profile_enhanced(
        self,
        pattern_scores: Dict[int, float],
        assessment_data: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        """
        Enhanced profiling that handles both clinical and healthy cases
        Returns appropriate profile based on score distribution
        """
        
        # Check if this is a healthy case
        max_score = max(pattern_scores.values())
        avg_score = sum(pattern_scores.values()) / len(pattern_scores)
        clinical_count = sum(1 for s in pattern_scores.values() if s >= 6.0)
        ...
        if is_healthy_case:
            # Use specialized healthy profiler
            return {
                "profile_category": "WELLNESS",
                "wellness_profile": self.healthy_profiler.generate_healthy_case_report(
                    pattern_scores,
                    assessment_data or {}
                ),
                "show_clinical_data": False,
                "user_message_type": "celebration_and_growth"
            }
        
        else:
            # Use standard clinical profiler
            clinical_profile = self.universal_profiler.create_universal_profile(
                pattern_scores
            )
            
            return {
                "profile_category": "CLINICAL",
                "clinical_profile": clinical_profile,
                "show_clinical_data": True,
                "user_message_type": "support_and_intervention"
            }
...
# ============================================================================
# VISUALIZATION DATA FOR HEALTHY CASES
# ============================================================================

class HealthyVisualizationGenerator:
    """Generate visualization data for healthy profiles"""
    
    @staticmethod
    def generate_wellness_radar_chart(pattern_scores: Dict[int, float]) -> Dict:
        """
        Radar chart for wellness - emphasizes strengths
        Green color scheme instead of red/yellow
        """
        
        pattern_names = HealthyCaseProfiler().pattern_names
        
        labels = [pattern_names[i] for i in range(1, 11)]
        
        # Invert scores for visualization (10 = best, 0 = worst)
        inverted_scores = [10 - pattern_scores.get(i, 5) for i in range(1, 11)]
        
        colors = []
        for score in inverted_scores:
            if score >= 7.5:  # Original 2.5 or better
                colors.append("#10b981")  # Dark green - Excellent
            elif score >= 7.0:  # Original 3.0
                colors.append("#22c55e")  # Green - Very good
            elif score >= 6.5:  # Original 3.5
                colors.append("#4ade80")  # Light green - Good
            else:  # Original 4.0
                colors.append("#84cc16")  # Yellow-green - Healthy
        
        return {
            "type": "radar",
            "title": "Your Wellness Profile",
            "subtitle": "Higher scores = Better functioning",
            "labels": labels,
            "datasets": [{
                "label": "Your Strengths",
                "data": inverted_scores,
    ...
    @staticmethod
    def generate_strength_bars(strengths: List[Dict]) -> Dict:
        """Generate bar chart emphasizing top strengths"""
        
        labels = [s["superpower"] for s in strengths]
        scores = [10 - s["score"] for s in strengths]  # Invert for visualization
        
        return {
            "type": "horizontalBar",
            "title": "Your Top 3 Superpowers",
            "labels": labels,
            "datasets": [{
...
class MasterProfilingSystem:
    ...
    def generate_complete_profile(
        ...
        pattern_scores = complete_analytics["pattern_scores"]
        #universal_profile = self.universal_profiler.create_universal_profile(pattern_scores)
        is_healthy_case = self._is_healthy_case(pattern_scores)
        
        # Step 3: Route to appropriate profiler
        if is_healthy_case:
            # ← NEW: Use wellness profiler for healthy cases
            return self._generate_wellness_profile(
                complete_analytics, 
                responses
            )
        else:
            # Existing clinical profiler (unchanged)
            return self._generate_clinical_profile(
                complete_analytics,
                responses)

    def _is_healthy_case(self, pattern_scores: Dict[int, float]) -> bool:
        """
        NEW METHOD: Detect if this is a healthy case
        
        Criteria for healthy case:
        - No patterns >= 6.0 (no clinical concerns)
        - Average score < 4.0 (adaptive range)
        - Max score < 5.0 (no borderline clinical)
        """
        max_score = max(pattern_scores.values())
        avg_score = sum(pattern_scores.values()) / len(pattern_scores)
        clinical_count = sum(1 for s in pattern_scores.values() if s >= 6.0)
    ...
    def _generate_wellness_profile(
        self,
        complete_analytics: Dict,
        responses: Dict[int, Any]
    ) -> Dict[str, Any]:
        """
        NEW METHOD: Generate wellness profile for healthy cases
        """
        
        pattern_scores = complete_analytics["pattern_scores"]
        
        # Generate wellness-specific report
        wellness_report = self.healthy_profiler.generate_healthy_case_report(
            pattern_scores,
            complete_analytics
        )
        
        # Generate wellness visualizations
        wellness_viz = self._generate_wellness_visualizations(pattern_scores)
        
        # Quick access summary for healthy cases
        executive_summary = {
            "profile_category": "WELLNESS",
            "profile_type": wellness_report["profile_classification"]["profile_type"],
            "health_score": wellness_report["profile_classification"]["health_metrics"]["average_score"],
            "percentile": wellness_report["profile_classification"]["percentile_rank"]["percentile"],
            "top_superpower": wellness_report["your_superpowers"][0]["superpower"],
            "primary_recommendation": wellness_report["recommendations"]["primary_recommendation"]
        }
        
        return {
            "profile_id": f"WELLNESS_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
            "generated_timestamp": datetime.now().isoformat(),
            "profile_category": "WELLNESS",
            
            # Analytics (still useful for trends)
            "complete_analytics": complete_analytics,
            
            # Wellness-specific content
            "wellness_profile": wellness_report,
            
            # Wellness visualizations
            "visualizations": wellness_viz,
            
            # Reports
            "client_report": self._generate_wellness_client_report(wellness_report),
            "therapist_note": self._generate_wellness_therapist_note(wellness_report),
            
            # Quick access
            "executive_summary": executive_summary
        }

    def _generate_clinical_profile(
        self,
        complete_analytics: Dict,
        responses: Dict[int, Any]
    ) -> Dict[str, Any]:
        """
        EXISTING METHOD: Keep original clinical profiling
        Just extracted into separate method for clarity
        """
        
        pattern_scores = complete_analytics["pattern_scores"]
        
        # Generate universal profile (existing code)
        universal_profile = self.universal_profiler.create_universal_profile(pattern_scores)
        ...
        return {
            "profile_id": f"CLINICAL_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
            "generated_timestamp": datetime.now().isoformat(),
            "profile_category": "CLINICAL",
            # Core analytics
            "complete_analytics": complete_analytics,
    ...
    def _generate_wellness_visualizations(
        self,
        pattern_scores: Dict[int, float]
    ) -> Dict[str, Any]:
        """
        NEW METHOD: Generate wellness-focused visualizations
        """
        
        # Use green color schemes instead of red/yellow
        wellness_viz_generator = HealthyVisualizationGenerator()
        
        return {
            "wellness_radar": wellness_viz_generator.generate_wellness_radar_chart(pattern_scores),
            "strength_bars": wellness_viz_generator.generate_strength_bars(
                self.healthy_profiler.identify_top_strengths(pattern_scores)
            ),
            # Optionally include standard charts for completeness
            "standard_radar": self.spectrum_visualizer.generate_radar_chart_data(pattern_scores),
            "standard_bars": self.spectrum_visualizer.generate_horizontal_bar_data(pattern_scores)
        }
    
    def _generate_wellness_client_report(
        self,
        wellness_report: Dict
    ) -> Dict[str, Any]:
        """
        NEW METHOD: Generate client-facing wellness report
        """
        
        return {
            "report_type": "YOUR WELLNESS BLUEPRINT",
            "celebration": wellness_report["celebration_message"],
            "your_profile": wellness_report["profile_classification"],
            "your_superpowers": wellness_report["your_superpowers"],
            "growth_opportunities": wellness_report["growth_opportunities"],
            "recommendations": wellness_report["recommendations"],
            "what_this_means": wellness_report["what_this_means"],
            "next_steps": wellness_report["next_steps"],
            "all_scores": wellness_report["all_pattern_scores"]
        }
    
    def _generate_wellness_therapist_note(
        self,
        wellness_report: Dict
    ) -> Dict[str, Any]:
        """
        NEW METHOD: Generate therapist note for healthy case
        """
        
        profile = wellness_report["profile_classification"]
        strengths = wellness_report["your_superpowers"]
        growth = wellness_report["growth_opportunities"]
        
        return {
            "clinical_note": "WELLNESS CASE - No clinical intervention needed",
            "profile_type": profile["profile_type"],
            "health_metrics": profile["health_metrics"],
            "percentile": profile["percentile_rank"],
            "top_strengths": [s["superpower"] for s in strengths],
            "growth_areas": [g["pattern_name"] for g in growth] if growth else ["None identified"],
            "recommendation": wellness_report["recommendations"]["primary_recommendation"],
            "suggested_support": wellness_report["recommendations"]["session_type"],
            "notes": [
                "Client demonstrates excellent psychological health",
                "Focus on optimization rather than intervention",
                "Consider coaching/mentoring rather than therapy",
                "Good candidate for advanced personal development work"
            ]
        }
...
class AnalyticsEngine:
    ...
    def analyze_digital_emotions(self, responses: Dict[int, Any]) -> Dict[str, Any]:
        ...
        # Q5 = Screen time hours (slider 0-16)
        # Q49 = Social media percentage (slider 0-100)
        response_5 = responses.get(5, 4)
        screen_time = 4 if response_5 in ['Not applicable', None, ''] else float(response_5)
        response_49 = responses.get(49, 50)
        social_media_pct = 50 if response_49 in ['Not applicable', None, ''] else float(response_49)
    ...
    def _calculate_resistance_intensity(self, responses: Dict[int, Any]) -> str:
        """Calculate overall resistance intensity"""
        # Q71: Change belief (lower = more resistance)
        response_71 = responses.get(71, 5)
        change_belief = 5 if response_71 in ['Not applicable', None, ''] else float(response_71)
    ...
    def assess_transformation_readiness(self, responses: Dict[int, Any]) -> Dict[str, Any]:
        ...
        # Q71: Change belief score
        response_71_readiness = responses.get(71, 5)
        change_belief = 5 if response_71_readiness in ['Not applicable', None, ''] else float(response_71_readiness)
...
class HealthyCaseProfiler:
    """
    Specialized profiling for individuals in adaptive range (scores 2.0-4.0)
    Provides growth-oriented, strengths-based analysis
    """
    
    def __init__(self):
        self.healthy_threshold = 4.0
        ...
        self.pattern_names = {
            ...
            3: "Trust & Connection",
            4: "Nuanced Thinking",
            5: "Being & Accomplishment Balance",
            6: "Authentic Self-Expression",
            7: "Self-Care & Boundaries",
            8: "Personal Autonomy",
            9: "Consistent Boundaries",
            10: "Digital-Life Integration"
        }
        
        # Strengths descriptions for adaptive patterns
        self.strength_descriptions = {
            1: {
                "superpower": "Joy Resilience",
                "what_this_means": "You naturally embrace positive experiences without waiting for the other shoe to drop",
                "growth_edge": "Deepening your capacity to savor and extend joyful moments",
                "optimization": "Practice gratitude journaling to amplify natural positivity"
            },
            2: {
                "superpower": "Collaborative Intelligence",
                "what_this_means": "You navigate disagreements with curiosity rather than defensiveness",
                "growth_edge": "Becoming even more skilled at win-win conflict resolution",
                "optimization": "Study advanced negotiation techniques to enhance natural collaboration"
            },
            3: {
                "superpower": "Open Trust",
                "what_this_means": "You balance healthy skepticism with openness to genuine connection",
                "growth_edge": "Refining your discernment about who deserves deeper trust",
                "optimization": "Learn to read micro-expressions for even better social calibration"
            },
            4: {
                "superpower": "Complexity Navigation",
                "what_this_means": "You naturally see nuance and hold multiple perspectives simultaneously",
                "growth_edge": "Helping others develop this skill through teaching or mentoring",
                "optimization": "Study systems thinking to leverage your natural both/and mindset"
            },
            5: {
                "superpower": "Intrinsic Worth",
                "what_this_means": "You know your value exists independent of achievements",
                "growth_edge": "Modeling this balance for others who struggle with productivity addiction",
                "optimization": "Explore contemplative practices to deepen your sense of being"
            },
            6: {
                "superpower": "Authentic Consistency",
                "what_this_means": "You're the same person across all contexts - genuine and integrated",
                "growth_edge": "Supporting others in finding their authentic voice",
                "optimization": "Write about your authenticity journey to help others"
            },
            7: {
                "superpower": "Reciprocal Care",
                "what_this_means": "You naturally balance giving to others with taking care of yourself",
                "growth_edge": "Teaching boundary-setting skills to those who struggle",
                "optimization": "Develop a self-care practice you can share with your community"
            },
            8: {
                "superpower": "Self-Directed Living",
                "what_this_means": "You honor family while living your own authentic path",
                "growth_edge": "Helping others navigate family expectations with grace",
                "optimization": "Mentor younger people dealing with family pressure"
            },
            9: {
                "superpower": "Universal Boundaries",
                "what_this_means": "Your boundaries stay consistent regardless of who you're with",
                "growth_edge": "Teaching boundary skills in contexts where you're naturally strong",
                "optimization": "Study assertiveness training to formalize your natural skills"
            },
            10: {
                "superpower": "Digital-Physical Balance",
                "what_this_means": "You use technology intentionally without letting it control you",
                "growth_edge": "Helping others achieve healthy digital boundaries",
                "optimization": "Share your digital wellness strategies with your community"
            }
        }
    
    def classify_healthy_profile(
        self, 
        pattern_scores: Dict[int, float]
    ) -> Dict[str, Any]:
        """
        Classify type of healthy functioning
        """
        ...
        # Count optimal vs adaptive
        optimal_count = sum(1 for s in scores_list if s < 3.0)
        adaptive_count = sum(1 for s in scores_list if 3.0 <= s < 4.0)
        
        # Determine profile type
        if avg_score < 2.5 and optimal_count >= 8:
            profile_type = "EXCEPTIONAL FUNCTIONING"
            profile_description = (
                "You're operating at an exceptionally high level of psychological health. "
                "Your patterns show optimal functioning across nearly all domains. "
                "You're in the top 5% of emotional intelligence and self-awareness."
            )
            profile_color = "#10b981"  # Green
            
        elif avg_score < 3.0 and optimal_count >= 6:
            profile_type = "OPTIMAL WELL-BEING"
            profile_description = (
                "You demonstrate excellent psychological health and emotional maturity. "
                "You've clearly done significant personal growth work. "
                "You're functioning in the optimal range across most life areas."
            )
            profile_color = "#22c55e"  # Light green
            
        elif adaptive_count >= 6:
            profile_type = "HEALTHY ADAPTIVE FUNCTIONING"
            profile_description = (
                "You're in a healthy adaptive range - functioning well with normal "
                "variations. You have solid coping skills and emotional regulation. "
                "There's room for growth and optimization."
            )
            profile_color = "#4ade80"  # Lighter green
            
        elif max_score < 5.0:
            profile_type = "BALANCED WELLNESS"
            profile_description = (
                "You show balanced wellness across life domains. Your patterns are "
                "within healthy ranges with some areas slightly more developed than others. "
                "You're well-positioned for growth-oriented work."
            )
            profile_color = "#84cc16"  # Yellow-green
            
        else:
            profile_type = "WELLNESS WITH GROWTH AREAS"
            profile_description = (
                "You're generally healthy with 1-2 specific areas that could benefit "
                "from focused attention. This is normal - everyone has growth edges. "
                "Your foundation is solid for targeted optimization work."
            )
            profile_color = "#a3e635"  # Light lime
        ...
        return {
            ...
            "percentile_rank": self._calculate_percentile(avg_score)
        }
    
    def _calculate_percentile(self, avg_score: float) -> Dict[str, Any]:
        """
        Calculate where this person ranks in population
        Based on normal distribution assumptions
        """
        
        if avg_score < 2.5:
            percentile = 95
            interpretation = "Top 5% of psychological health"
        elif avg_score < 3.0:
            percentile = 85
            interpretation = "Top 15% - Excellent functioning"
        elif avg_score < 3.5:
            percentile = 70
            interpretation = "Top 30% - Very healthy"
        else:
            percentile = 60
            interpretation = "Above average - Healthy range"
    ...
    def identify_top_strengths(
        ...
        """
        Identify person's top 3 psychological strengths
        These are their superpowers
        """
        
        # Sort patterns by lowest score (best functioning)
        sorted_patterns = sorted(
            pattern_scores.items(),
            key=lambda x: x[1]
        )
        ...
        strengths = []
        for pattern_id, score in top_3:
            strength_info = self.strength_descriptions.get(pattern_id, {})
            
            strengths.append({
                "pattern_id": pattern_id,
                "pattern_name": self.pattern_names[pattern_id],
                "score": round(score, 1),
                "superpower": strength_info.get("superpower", "Natural strength"),
                "what_this_means": strength_info.get("what_this_means", "You excel in this area"),
                "how_to_leverage": strength_info.get("growth_edge", "Continue developing this strength"),
                "rank": strengths.__len__() + 1
            })
    ...
    def identify_growth_edges(
        ...
        """
        Identify areas for growth/optimization (not problems!)
        Frame as opportunities, not deficits
        """
        
        # Get patterns in 3.5-4.0 range - good but could be better
        growth_edges = []
        ...
        for pattern_id, score in pattern_scores.items():
            if 3.5 <= score < 4.5:
                ...
                growth_edges.append({
                    ...
                    "pattern_name": self.pattern_names[pattern_id],
                    "score": round(score, 1),
                    "current_state": "Good - functioning well in this area",
                    "growth_opportunity": strength_info.get("growth_edge", "Room for enhancement"),
                    "optimization_strategy": strength_info.get("optimization", "Continue developing this skill"),
                    "potential_impact": "Moving from good to great in this domain"
                })
        
        # Sort by score (highest first - most room for growth)
        growth_edges.sort(key=lambda x: x['score'], reverse=True)
        
        return growth_edges[:3]  # Top 3 growth edges
    ...
    def generate_wellness_recommendations(
        ...
        """
        Generate growth-oriented recommendations for healthy individuals
        Not therapy - enhancement and optimization
        """
        
        profile_type = profile_classification["profile_type"]
        
        if "EXCEPTIONAL" in profile_type:
            primary_recommendation = "CONTRIBUTION & TEACHING"
            recommendation_text = (
                "You're operating at such a high level that your next growth edge is "
                "sharing your wisdom with others. Consider coaching, mentoring, or "
                "teaching roles where you can help others develop these skills."
            )
            session_type = "Optional: Peak Performance Coaching"
            
        elif "OPTIMAL" in profile_type:
            primary_recommendation = "ADVANCED OPTIMIZATION"
            recommendation_text = (
                "Continue your excellent trajectory with advanced personal development work. "
                "Focus on integration, refinement, and perhaps exploring contemplative practices "
                "or leadership development."
            )
            session_type = "Optional: 1-2 optimization sessions for specific goals"
            
        elif "HEALTHY ADAPTIVE" in profile_type:
            primary_recommendation = "TARGETED GROWTH"
            recommendation_text = (
                "You're in a great position for focused growth work. Pick 1-2 areas where "
                "you'd like to level up from 'good' to 'excellent'. This is about optimization, "
                "not fixing problems."
            )
            session_type = "Recommended: 1-2 enhancement sessions for specific areas"
            
        else:  # BALANCED or WELLNESS WITH GROWTH
            primary_recommendation = "PREVENTIVE OPTIMIZATION"
            recommendation_text = (
                "Address your 1-2 growth edges before they become larger patterns. "
                "This is preventive work - handling small things before they become big things. "
                "Much easier than waiting for crisis."
            )
            session_type = "Recommended: 2-3 sessions for specific pattern optimization"
        
        return {
            "primary_recommendation": primary_recommendation,
            "recommendation_text": recommendation_text,
            "session_type": session_type,
            "specific_actions": self._generate_specific_actions(strengths, growth_edges),
            "maintenance_strategies": self._generate_maintenance_strategies(strengths),
            "growth_resources": self._generate_growth_resources(growth_edges)
        }
    ...
    def _generate_specific_actions(
        ...
        if strengths:
            ...
            actions.append(
                f"LEVERAGE YOUR SUPERPOWER: Your {top_strength['superpower']} is exceptional. "
                f"Consider how you can use this strength to support others or advance your goals."
            )
        
        # Address top growth edge
        if growth_edges:
            top_growth = growth_edges[0]
            actions.append(
                f"OPTIMIZATION OPPORTUNITY: Enhance your {top_growth['pattern_name']} "
                f"through {top_growth['optimization_strategy']}."
            )
        
        # General wellness
        actions.extend([
            "MAINTAIN YOUR FOUNDATION: Continue whatever practices have gotten you to this healthy state",
            "EXPLORE GROWTH: Consider advanced personal development work - you have the foundation for deep growth",
            "SHARE YOUR JOURNEY: Your wellness journey could inspire and help others"
        ])
        
        return actions[:5]  # Top 5 actions
    
    def _generate_maintenance_strategies(
        self,
        strengths: List[Dict]
    ) -> List[str]:
        """Generate strategies to maintain current health"""
        
        return [
            "Continue your current self-care practices - they're clearly working",
            "Regular check-ins with yourself (monthly self-reflection)",
            "Maintain boundaries that protect your well-being",
            "Stay connected to supportive relationships",
            "Keep learning and growing - stagnation is the enemy of wellness"
        ]
    
    def _generate_growth_resources(
        self,
        growth_edges: List[Dict]
    ) -> List[Dict[str, str]]:
        """Generate specific resources for growth areas"""
        
        resources = [
            {
                "type": "Book",
                "title": "Peak Performance: Elevate Your Game",
                "why": "Advanced optimization strategies for high-functioning individuals"
            },
            {
                "type": "Practice",
                "title": "Advanced Mindfulness or Contemplative Practice",
                "why": "Deepening self-awareness from healthy baseline"
            },
            {
                "type": "Community",
                "title": "Mastermind or High-Performing Peer Group",
                "why": "Surround yourself with others at your level or higher"
            }
        ]
        
        return resources
    
    def generate_healthy_case_report(
        self,
        pattern_scores: Dict[int, float],
        assessment_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        MASTER FUNCTION: Generate complete report for healthy case
        This is what gets shown instead of "no results"
        """
        
        # 1. Classify profile
        profile_classification = self.classify_healthy_profile(pattern_scores)
        
        # 2. Identify strengths
        top_strengths = self.identify_top_strengths(pattern_scores)
        
        # 3. Identify growth edges
        growth_edges = self.identify_growth_edges(pattern_scores)
        
        # 4. Generate recommendations
        recommendations = self.generate_wellness_recommendations(
            profile_classification,
            top_strengths,
            growth_edges
        )
        
        # 5. Create celebration message
        celebration = self._generate_celebration_message(profile_classification)
        
        return {
            "report_type": "WELLNESS PROFILE",
            "profile_classification": profile_classification,
            "celebration_message": celebration,
            "your_superpowers": top_strengths,
            "growth_opportunities": growth_edges,
            "all_pattern_scores": {
                pid: {
                    "name": self.pattern_names[pid],
                    "score": round(score, 1),
                    "status": self._get_pattern_status(score)
                }
                for pid, score in pattern_scores.items()
            },
            ...
            "what_this_means": {
                "the_good_news": (
                    "You're in excellent psychological health. This assessment shows you're "
                    "functioning well above average in most or all life areas. You've clearly "
                    "done significant personal growth work or have natural emotional intelligence."
                ),
                "what_now": (
                    "You don't need therapy or clinical intervention. Instead, you can focus on "
                    "optimization, advanced growth, or using your strengths to help others. "
                    "Think of this as going from 'good' to 'great' rather than 'broken' to 'fixed'."
                ),
                "your_advantage": (
                    "Because you're starting from a healthy baseline, any growth work you do will "
                    "be exponentially more effective. You have the foundation for rapid advancement."
                )
            },
            "next_steps": self._generate_healthy_next_steps(profile_classification),
    ...
    def _generate_celebration_message(
        ...
        """Generate celebratory message for healthy individuals"""
        
        profile_type = profile_classification["profile_type"]
        percentile = profile_classification["percentile_rank"]["percentile"]
        
        celebrations = {
            "EXCEPTIONAL FUNCTIONING": (
                f"🌟 EXCEPTIONAL! You're in the top {100-percentile}% of psychological health. "
                f"This is truly remarkable - you've achieved what most people work years to develop."
            ),
            "OPTIMAL WELL-BEING": (
                f"✨ EXCELLENT! You're functioning in the top {100-percentile}% - this is outstanding. "
                f"Your self-awareness and emotional intelligence are highly developed."
            ),
            "HEALTHY ADAPTIVE FUNCTIONING": (
                f"💚 HEALTHY! You're in the top {100-percentile}% with solid psychological health. "
                f"You have excellent coping skills and emotional maturity."
            ),
            "BALANCED WELLNESS": (
                f"🌱 BALANCED! You show healthy functioning across life domains. "
                f"Your baseline is strong - perfect foundation for growth."
            ),
            "WELLNESS WITH GROWTH AREAS": (
                f"✅ HEALTHY WITH OPPORTUNITY! You're functioning well overall "
                f"with specific areas where you can level up even further."
            )
        }
        
        return celebrations.get(
            profile_type,
            "You're in healthy psychological functioning - this is excellent!"
        )
    
    def _get_pattern_status(self, score: float) -> str:
        """Get status description for a pattern score"""
        
        if score < 2.5:
            return "OPTIMAL - Exceptional functioning"
        elif score < 3.0:
            return "EXCELLENT - Very healthy"
        elif score < 3.5:
            return "GOOD - Healthy range"
        elif score < 4.0:
            return "ADAPTIVE - Functioning well"
        else:
            return "GROWTH OPPORTUNITY - Room for enhancement"
    
    def _generate_healthy_next_steps(
        self,
        profile_classification: Dict
    ) -> Dict[str, Any]:
        """Generate next steps for healthy individuals"""
        
        profile_type = profile_classification["profile_type"]
        
        if "EXCEPTIONAL" in profile_type:
            return {
                "immediate_action": "Consider contribution/teaching opportunities",
                "optional_support": "Peak performance coaching if interested in specific optimization",
                "timeline": "Self-directed - you're equipped to continue independently",
                "cost": "Optional coaching: 1-2 sessions if desired"
            }
        
        elif "OPTIMAL" in profile_type:
            return {
                "immediate_action": "Identify 1-2 areas where you want to go from great to extraordinary",
                "optional_support": "1-2 optimization sessions for specific goals",
                "timeline": "Self-directed with optional professional support",
                "cost": "Optional: à¸¿1,500-3,000 for 1-2 enhancement sessions"
            }
        
        else:  # HEALTHY ADAPTIVE, BALANCED, or WELLNESS WITH GROWTH
            return {
                "immediate_action": "Review your growth edges and pick 1-2 to focus on",
                "recommended_support": "1-2 targeted sessions for specific pattern enhancement",
                "timeline": "2-4 weeks for focused optimization work",
                "cost": "Recommended: à¸¿3,000-4,500 for 2-3 optimization sessions"
            }
...
class AssessmentConfig:
    ...
    @staticmethod
    def get_healthy_profiler():  # ← ADD THIS
        return HealthyCaseProfiler()
    
    def get_master_profiler():  # ← ADD THIS
        return MasterProfilingSystem()
