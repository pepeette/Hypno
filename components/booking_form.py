...
#                           text-decoration: none; padding: var(--space-sm) var(--space-md); 
#                           border-radius: var(--radius-sm); font-weight: 600; 
#                           font-size: var(--font-size-normal); transition: var(--transition);
#                           box-shadow: var(--shadow-sm); text-align: center; width: 100%;
#                           box-sizing: border-box; margin: var(--space-xs) 0; 
#                           border: 2px solid var(--border); min-height: 2.5rem;
#                           vertical-align: middle; line-height: 1.6; cursor: pointer;">
#                     💬 Message on WhatsApp
#                 </a>
#             """, unsafe_allow_html=True)
...
class BookingForm:
    ...
    def render(self, form_title=None, form_description=None):
        ...
        # Basic mobile-friendly CSS - no JavaScript
        st.markdown("""
        <style>
        /* Basic mobile-friendly styling - no JavaScript */
        .stTextArea textarea,
        .stTextInput input,
        .stNumberInput input,
        textarea,
    ...
    def _render_form(self):
        ...
        # Open calendar using Streamlit's native link
        if st.session_state.get('trigger_calendar', False):
            st.markdown(f'<a href="{self.discovery_url}" target="_blank">Click here to open the calendar</a>', unsafe_allow_html=True)
            # Clear the trigger
            st.session_state.trigger_calendar = False
